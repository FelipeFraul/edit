export type MediaItem = {
  videoSrc?: string
  poster?: string
  who: string
  when: string
  category: string
  title?: string
  subtitle?: string
}

export type HeroVariant = {
  pos: -3 | -2 | -1 | 0 | 1 | 2 | 3
  mode: "split" | "ads" | "entertainment"
  kicker: string
  title?: string
  animatedPrefix?: string
  animatedWords?: string[]
  tagline: string
  mobileLines?: string[]
  ctaHref: string
  topCtaLabel?: string
  topCtaHref?: string
  media: MediaItem
  mobileMedia?: MediaItem
  bgImage: string
  mobileBgImage?: string
}

export const getVariantMedia = (variant: HeroVariant, isMobileViewport: boolean) =>
  isMobileViewport && variant.mobileMedia ? variant.mobileMedia : variant.media

export const heroVariants: HeroVariant[] = [
  {
    pos: -3,
    mode: "ads",
    kicker: "DIREÇÃO, PRODUÇÃO E MIXAGEM",
    tagline: "Técnica comercial viva e descontraída para a marca Giraffas.",
    ctaHref: "#ads-3",
    topCtaLabel: "VER MÍDIA",
    topCtaHref: "#midia-3",
    media: {
      videoSrc: "https://player.vimeo.com/video/1162811824?",
      poster: "",
      who: "BUDŌ",
      when: "2024",
      category: "ANIMAÇÃO",
      title: "UMA PROMOÇÃO PARA DAR UM SUSTO NA SUA FOME",
      subtitle: "Campanha de lançamento para um novo público urbano.",
    },
    mobileMedia: {
      videoSrc: "https://player.vimeo.com/video/1162811824?",
      poster: "",
      who: "BUDŌ",
      when: "2024",
      category: "ANIMAÇÃO",
      title: "UMA PROMOÇÃO PARA DAR UM SUSTO NA SUA FOME",
      subtitle: "Campanha de lançamento para um novo público urbano.",
    },
    bgImage: "/assets/hero_budo_desk.webp",
    mobileBgImage: "/assets/hero_budo_mobile.webp",
  },
  {
    pos: -2,
    mode: "ads",
    kicker: "DIREÇÃO, PRODUÇÃO E MIXAGEM",
    tagline:
      "Técnica, direção e narrativa alinhadas ao compromisso da Ambev de conectar pessoas.",
    ctaHref: "#ads-2",
    topCtaLabel: "VER MÍDIA",
    topCtaHref: "#midia-2",
    media: {
      videoSrc: "https://player.vimeo.com/video/1168962471?",
      poster: "",
      who: "BUZZMONITOR",
      when: "2024",
      category: "Publicidade",
      title: "COISA COMPLETA E FLEXÍVEL PRA CAR!@#$%^&*.",
      subtitle: "Reposicionamento com foco em impacto e clareza.",
    },
    mobileMedia: {
      videoSrc: "https://player.vimeo.com/video/1168962471?",
      poster: "",
      who: "BUZZMONITOR",
      when: "2024",
      category: "Publicidade",
      title: "COISA COMPLETA E FLEXÍVEL PRA CAR!@#$%^&*.",
      subtitle: "Reposicionamento com foco em impacto e clareza.",
    },
    bgImage: "/assets/hero_buzz_desk.webp",
    mobileBgImage: "/assets/hero_buzz_mobile.webp",
  },
  {
    pos: -1,
    mode: "ads",
    kicker: "DIREÇÃO, PRODUÇÃO E MIXAGEM",
    tagline:
      "Técnica, direção e narrativa alinhadas ao padrão Globo de excelência.",
    ctaHref: "#ads-1",
    topCtaLabel: "VER MÍDIA",
    topCtaHref: "#midia-1",
    media: {
      videoSrc: "https://player.vimeo.com/video/1168960953?",
      poster: "",
      who: "COCA-COLA",
      when: "2024",
      category: "Publicidade",
      title: "UMA EXPERIÊNCIA TODA SENSORIAL E IMERSIVA DE NATAL.",
      subtitle: "",
    },
    mobileMedia: {
      videoSrc: "https://player.vimeo.com/video/1168960953?",
      poster: "/assets/hero_coca_cola_mobile.webp",
      who: "COCA-COLA",
      when: "2024",
      category: "Publicidade",
      title: "UMA EXPERIÊNCIA TODA SENSORIAL E IMERSIVA DE NATAL.",
      subtitle: "",
    },
    bgImage: "/assets/hero_coca_cola_desk.webp",
    mobileBgImage: "/assets/hero_coca_cola_mobile.webp",
  },
  {
    pos: 0,
    mode: "split",
    kicker: "ESTÚDIO DE PRODUÇÃO CRIATIVA",
    title: "EXPERIÊNCIA SONORA QUE TRANSFORMA IMAGEM BRUTA EM FILME QUE",
    animatedPrefix: "EXPERIÊNCIA SONORA QUE TRANSFORMA IMAGEM BRUTA EM FILME QUE",
    animatedWords: [
      "MARCA",
      "IMPACTA",
      "INSPIRA",
      "PROVOCA",
      "RESSOA",
      "MOBILIZA",
    ],
    tagline: "Unimos técnica, estética e narrativa para dar vida às marcas.",
    mobileLines: [""],
    ctaHref: "#split",
    topCtaLabel: "AGÊNCIA DE VOZES",
    topCtaHref: "#split",
    media: {
      videoSrc: "",
      poster: "",
      who: "EDIT GROUP",
      when: "2025",
      category: "Publicidade",
      title: "EXPERIÊNCIAS QUE CONECTAM PESSOAS POR",
      subtitle: "Narrativas de alto impacto no cruzamento de mídia e arte.",
    },
    mobileMedia: {
      videoSrc: "",
      poster: "",
      who: "EDIT GROUP",
      when: "2025",
      category: "Publicidade",
      title: "EXPERIÊNCIAS QUE CONECTAM PESSOAS POR",
      subtitle: "Narrativas de alto impacto no cruzamento de mídia e arte.",
    },
    bgImage:
      "",
    mobileBgImage: "",
  },
  {
    pos: 1,
    mode: "entertainment",
    kicker: "ESTÚDIO DE PRODUÇÃO CRIATIVA",
    tagline: "Unimos técnica, estética e narrativa para dar vida às marcas.",
    ctaHref: "#ent-1",
    topCtaLabel: "VER MÍDIA",
    topCtaHref: "#midia-ent-1",
    media: {
      videoSrc: "https://player.vimeo.com/video/1168958825?",
      poster: "",
      who: "SATA",
      when: "2024",
      category: "Publicidade",
      title: "TEMPORADA 1 - ERA LUZ QUE ME FALTAVA. E AGORA?",
      subtitle: "Experiência audiovisual para lançamento global.",
    },
    mobileMedia: {
      videoSrc: "",
      poster: "",
      who: "SATA",
      when: "2024",
      category: "Publicidade",
      title: "TEMPORADA 1 - ERA LUZ QUE ME FALTAVA. E AGORA?",
      subtitle: "Experiência audiovisual para lançamento global.",
    },
    bgImage: "/assets/hero_sata_desk.webp",
    mobileBgImage: "/assets/hero_sata_mobile.webp",
  },
  {
    pos: 2,
    mode: "entertainment",
    kicker: "ESTÚDIO DE PRODUÇÃO CRIATIVA",
    tagline: "Unimos técnica, estética e narrativa para dar vida às marcas.",
    ctaHref: "#ent-2",
    topCtaLabel: "VER MÍDIA",
    topCtaHref: "#midia-ent-2",
    media: {
      videoSrc: "https://player.vimeo.com/video/1168963549?",
      poster: "",
      who: "DETRAN PR",
      when: "2025",
      category: "PUBLICIDADE",
      title: "TODO MUNDO PODE MUDAR DE DIREÇÃO NA VIDA.",
      subtitle: "Nova identidade para streaming de autor.",
    },
    mobileMedia: {
      videoSrc: "https://player.vimeo.com/video/1168963549?",
      poster: "",
      who: "DETRAN PR",
      when: "2025",
      category: "PUBLICIDADE",
      title: "TODO MUNDO PODE MUDAR DE DIREÇÃO NA VIDA.",
      subtitle: "Nova identidade para streaming de autor.",
    },
    bgImage: "/assets/bg_hero_publicidade_ambev.webp",
    mobileBgImage: "/assets/bg_hero_publicidade_ambev.webp",
  },
  {
    pos: 3,
    mode: "entertainment",
    kicker: "ESTÚDIO DE PRODUÇÃO CRIATIVA",
    tagline: "Unimos técnica, estética e narrativa para dar vida às marcas.",
    ctaHref: "#ent-3",
    topCtaLabel: "VER MÍDIA",
    topCtaHref: "#midia-ent-3",
    media: {
      videoSrc: "",
      poster: "",
      who: "Test 3",
      when: "2024",
      category: "Entretenimento",
      title: "AFTERGLOW",
      subtitle: "Direção de arte para série original.",
    },
    mobileMedia: {
      videoSrc: "",
      poster: "",
      who: "Test 3",
      when: "2024",
      category: "Entretenimento",
      title: "AFTERGLOW",
      subtitle: "Direção de arte para série original.",
    },
    bgImage: "/assets/bg_hero_publicidade_giraffas.webp",
    mobileBgImage: "/assets/bg_hero_publicidade_giraffas.webp",
  },
]

const clampHeroPos = (pos: number) => Math.min(3, Math.max(-3, Math.round(pos))) as HeroVariant["pos"]

export const getHeroVariant = (pos: HeroVariant["pos"] | number) => {
  const snappedPos = clampHeroPos(pos)
  const exact = heroVariants.find((variant) => variant.pos === snappedPos)
  if (exact) return exact

  let best = heroVariants[0]
  let bestDist = Infinity
  for (const variant of heroVariants) {
    const d = Math.abs(variant.pos - snappedPos)
    if (d < bestDist) {
      bestDist = d
      best = variant
    }
  }
  return best
}
